...
// Debug - show paths on startup
var tempFolder = Folder.temp;
var commandPath = tempFolder.fsName.replace(/\\/g, "/") + "/ae_command.json";
var resultPath = tempFolder.fsName.replace(/\\/g, "/") + "/ae_mcp_result.json";
alert("Command file path: " + commandPath + "\nResult file path: " + resultPath);

// Command file path
function getCommandFilePath() {
    var tempFolder = Folder.temp;
    return tempFolder.fsName.replace(/\\/g, "/") + "/ae_command.json";
}

// Result file path
function getResultFilePath() {
    var tempFolder = Folder.temp;
    return tempFolder.fsName.replace(/\\/g, "/") + "/ae_mcp_result.json";
}
...
function executeCommand(command, args) {
    ...
    try {
        ...
        try {
            ...
            logToPanel("Writing result to: " + resultFile.fsName);
            resultFile.open("w");
            resultFile.write(result);
            resultFile.close();
            logToPanel("Result file written successfully");
        } catch (writeError) {
            logToPanel("ERROR writing result file: " + writeError.toString());
            alert("Failed to write result file: " + writeError.toString());
        }
        ...
        try {
            ...
            logToPanel("ERROR writing error to result file: " + e.toString());
        }
...
function checkForCommands() {
    ...
    try {
        var commandFile = new File(getCommandFilePath());
        logToPanel("Checking for commands at: " + commandFile.fsName + " - Exists: " + commandFile.exists);
        ...
        if (commandFile.exists) {
            try {
                ...
                if (content) {
                    logToPanel("Command file content: " + content);
                    ...
                    try {
                        ...
                        if (commandData.status === "pending") {
                            logToPanel("Found pending command: " + commandData.command);
                            ...
                            logToPanel("Command status is not 'pending': " + commandData.status);
                        }
                    } catch (parseError) {
                        logToPanel("Error parsing command file: " + parseError.toString());
                    }
                } else {
                    logToPanel("Command file is empty");
                }
            } catch (readError) {
                logToPanel("Error reading command file: " + readError.toString());
            }
        }
    } catch (e) {
